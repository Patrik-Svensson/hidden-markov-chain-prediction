...
public class Main
{
    ...
    public static double[][] calculateAlpha(){
		...
		// Compute alpha-i
		for(int t = 1; t <= T - 1; t++) // observation
		{
			c[t] = 0.0; 
			for(int i = 0; i <= N - 1 ; i++) // state
			{
				alpha[i][t] = 0.0;
				for(int j = 0; j <= N -1; j++) // marginalisring, komma till samma state på olika sätt
				{
    ...
    public static double[][] calculateBeta()
    {
        ...
        for (int i = 0; i <= N-1 ; i++) 
        {
            beta[i][T-1] = c[T-1]; // set 1s in last position
        }
